...
pub fn get_sprite_stream_response<R: Runtime>(
  ...
  let equipment_icons_state: State<EquipmentSpriteState> = handle.state::<EquipmentSpriteState>();

  let sprite_lock: MutexGuard<Option<String>> = equipment_icons_state.equipment_sprite_name.lock().unwrap();
  let preview_lock: MutexGuard<Option<Vec<u8>>> = equipment_icons_state.equipment_sprite_preview.lock().unwrap();
