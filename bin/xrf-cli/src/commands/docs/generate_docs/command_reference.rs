  ...
  pub fn from_group(group: &CommandGroup) -> Self {
    Self {
      ...
      commands: group
        .commands
        .iter()
        .map(|command| CommandReference::from_command(command.init()))
        .collect(),
    }
  }
}
...
impl CommandReference {
  ...
  pub fn from_command(mut command: Command) -> Self {
    ...
    let usage: String = Self::render_usage(&mut command);
...
mod tests {
  ...
  fn extracts_command_metadata() {
    ...
    assert!(reference.usage.starts_with("xrf-cli example-command"));
