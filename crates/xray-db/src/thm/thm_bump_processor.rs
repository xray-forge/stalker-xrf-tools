...
impl ThmBumpProcessor {
  ...
  fn read_bump(source: &Path) -> XRayResult<ThmBumpChunk> {
    ThmFile::read_from_path::<xray_chunk::XRayByteOrder, _>(&source)?
      .bump
...
mod tests {
  ...
  use xray_test_utils::utils::{
    get_absolute_test_resource_path, get_relative_test_sample_file_path, overwrite_test_relative_resource_as_file,
  };
  ...
  fn test_patch_bump_requires_bump_chunk() -> XRayResult {
    ...
    opaque_writer.write_all(&OPAQUE_PAYLOAD)?;
    opaque_writer
      .flush_chunk_into::<XRayByteOrder>(&mut overwrite_test_relative_resource_as_file(&filename)?, 0x0812)?;
