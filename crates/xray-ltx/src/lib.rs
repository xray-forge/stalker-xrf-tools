      ...
      Some('"') if self.opt.enabled_quote => {
        ...
        self.parse_str_until(&[Some('"')], false).and_then(|s| {
          self.bump(); // Eats the last "
                       // Parse until EOL
          self
            .parse_str_until_eol(cfg!(feature = "inline-comment"))
            .map(|x| s + &x)
        })
      }
      Some('\'') if self.opt.enabled_quote => {
...
";
    let ini = Ini::load_from_str(input).unwrap();
    assert_eq!(
      ini.get_from(Some("section name"), "name").unwrap(),
      "hello # abcdefg"
    );
    assert_eq!(
...
";
    ...
    assert_eq!(
      ini.get_from(Some("section name"), "address").unwrap(),
      "web#url"
    );
    assert_eq!(
      ini.get_from(Some("section name"), "phone").unwrap(),
      "01234"
    );
    assert_eq!(
      ini.get_from(Some("section name"), "phone2").unwrap(),
      "56789"
    );
    assert_eq!(
      ini.get_from(Some("section name"), "phone3").unwrap(),
      "43210"
    );
  }
