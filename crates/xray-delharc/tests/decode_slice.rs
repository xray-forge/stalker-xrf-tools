...
const TESTS_CASES: &[(&str, &[u8], CompressionMethod, u64)] = &[
  ...
  (
    "lz5",
    include_bytes!("decode/lz5.bin"),
    CompressionMethod::Lz5,
    1,
  ),
  #[cfg(feature = "lh1")]
  (
    "lh1",
    include_bytes!("decode/lh1.bin"),
    CompressionMethod::Lh1,
    1,
  ),
  (
    "lh5",
    include_bytes!("decode/lh5.bin"),
    CompressionMethod::Lh5,
    1,
  ),
  (
    "lh6",
    include_bytes!("decode/lh6.bin"),
    CompressionMethod::Lh6,
    1,
  ),
  (
    "lh7",
    include_bytes!("decode/lh7.bin"),
...
fn test_decode_slice() {
  ...
  for (name, file, compression, offset) in TESTS_CASES {
    ...
    for limit in [usize::max_value(), 128, 31, 3, 2, 1].iter().copied() {
      let mut slice = &file[..];
      let mut decoder =
        DecoderAny::new_from_compression(*compression, slice.take(compressed_size - offset));
