  ...
  fn next_bits(&mut self, n: u32) -> io::Result<BitBuf> {
    ...
    if bits_read < missing_bits {
      return Err(io::Error::new(
        ...
        "some bits are missing from stream",
      ));
    }
...
mod tests {
  ...
  fn bit_stream_works() {
    ...
    assert_eq!(
      brdr.read_bit().unwrap_err().kind(),
      io::ErrorKind::UnexpectedEof
    );
    assert_eq!(
      brdr.read_bits::<usize>(BITBUF_BITSIZE).unwrap_err().kind(),
      io::ErrorKind::UnexpectedEof
    );
    ...
    let mut somebits: &[u8] = &[
      0b01001100, 0b01110000, 0b11110000, 0b01111100, 0b00001111, 0b11000000, 0b01111111,
    ...
    match BITBUF_BITSIZE {
      ...
      64 => {
        assert_eq!(
          brdr.read_bits::<usize>(BITBUF_BITSIZE).unwrap(),
          0x0102030405060708
        );
      }
